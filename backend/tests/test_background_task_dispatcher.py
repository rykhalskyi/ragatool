...
class TestBackgroundTaskDispatcher(unittest.TestCase):
    ...
    def test_enqueue_task(self):
        with patch('app.internal.background_task_dispatcher.crud_task') as mock_crud_task:
            ...
            dispatcher = BackgroundTaskDispatcher(mock_message_hub, self.mock_db, num_workers=1)
            ...
            mock_crud_task.create_task.assert_called_once_with(self.mock_db, task_id, collection_id, task_name, ANY, "NEW")
            
            # Give the worker time to process the task
            time.sleep(0.2) # Increased sleep to ensure task completes
            
            mock_crud_task.update_task_status.assert_any_call(self.mock_db, task_id, "RUNNING")
            mock_crud_task.update_task_status.assert_any_call(self.mock_db, task_id, "COMPLETED")
    ...
    def test_cancel_queued_task(self):
        with patch('app.internal.background_task_dispatcher.crud_task') as mock_crud_task:
            ...
            mock_crud_task.create_task.assert_any_call(self.mock_db, long_task_id, collection_id, "long_task", unittest.mock.ANY, "NEW")
            
            # Enqueue the task to be cancelled
            task_id_to_cancel = dispatcher.add_task(collection_id, task_name, cancellable_dummy_task, 0.1)
            mock_crud_task.create_task.assert_any_call(self.mock_db, task_id_to_cancel, collection_id, task_name, unittest.mock.ANY, "NEW")
    ...
    def test_cancel_running_task(self):
        with patch('app.internal.background_task_dispatcher.crud_task') as mock_crud_task:
            ...
            mock_crud_task.create_task.assert_called_with(self.mock_db, task_id, collection_id, task_name, ANY, "NEW")
